...
//Display all the characters on the page and convert all the characters to clickable buttons 

function startGame () {
	$('.display').hide(); //this makes "Your Character" invsible until on-click button pushed
	for (var i = 0; i < characters.length; i++) {
		var b = $('<button>'); //declaring a variable, b, defined as a button 
		b.addClass('characterButton'); //using jquery to add the BUTTON class 'characterButton' 
		b.attr('name', characters[i].name); //this is creating a button attribute for the name of all the character
		b.attr('hp', characters[i].hp); //this is creating a button attribute for the hp of all the characters 
		b.attr('ap', characters[i].ap); //this is creating a button attribute for all the ap of all the characters
		b.attr('ca', characters[i].ca); //this is creating a button attribute for all the ca of all the characters 
		b.append("<p>" + characters[i].name + "</p><img src='" + characters[i].img + "' class='characterImage'><br><p class='hpDisplay'>HP: " + characters[i].hp + "</p>");
		$('#allCharacters').append(b);  //this appends all the characters names, images, and hp display to the buttons  
	}
	var p = $('<p>'); //still inside the startGame, we declare the variable p as the grabbable paragraph
	p.append('Choose your character.'); //so that we can use it here to append 'Choose your character'
	$('#gameText').append(p); //then we append the game text to the paragraph 
}
